...
// 计算 Vue 应用根目录，确保环境文件和输出路径不依赖命令执行目录。
const appRoot = fileURLToPath(new URL(".", import.meta.url));
// 定义注入所有 SCSS 文件和 Vue SCSS style 块的公共 Sass 能力入口。
      ...
      shouldAnalyze &&
        ...
        visualizer({
          ...
          filename: fileURLToPath(
            // 使用独立的 Vue 文件名，便于同时保留两套应用的分析结果。
            new URL("../../reports/bundle/{{APP_NAME}}.html", import.meta.url)
          ),
          // 在分析报告中计算 gzip 压缩体积。
          gzipSize: true,
    ...
    css: {
      ...
      preprocessorOptions: {
        ...
        scss: {
          ...
          api: "modern-compiler",
          // 自动注入公共 Sass 能力，使 Vue style 块无需重复 @use。
    ...
    build: {
      ...
      rolldownOptions: {
        ...
        output: {
          ...
          manualChunks(id) {
            // 判断当前模块是否属于 Vue、Vue Router 或 Pinia。
            if (
              // 匹配 Vue 核心运行时及其内部包。
